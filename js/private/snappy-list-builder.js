...
jQuery(document).ready(function($){
    ...
    */
    ...
    // for toggling all subscriber data on and off - function is in course, but doesn't work
    ...
    */
    ...
    // for toggling all subscriber data on and off - an alternative solution
    $(document).on('click', '#import_subscribers #import_form_2 #checkAll', function() {
        if( $(this).is(':checked') && slb_form_2_is_valid() ) {
            $('.show-only-on-valid', $import_form_2).show();
        } else {
            $('.show-only-on-valid', $import_form_2).hide();
        }
    ...
    function slb_get_form_2_html( subscribers ) {
        ...
        var tr = '<tr>';
        var th = '<th scope="col" class="manage-column check-column"><label><input type="checkbox" class="check-all" id="checkAll"></label></th>';
    ...
    function slb_get_selector( input_id, subscribers ) {
        ...
        // add our first option to select html
    ...
    // hint: returns a html tr formatted for WordPress  admin forms
    ...
    function slb_form_2_is_valid() {
        ...
        if( $('[name="slb_fname_column"] option:checked', $import_form_2).val() == '' )
            is_valid = false;

        // check if no lname column is selected
        if( $('[name="slb_lname_column"] option:checked', $import_form_2).val() == '' )
            is_valid = false;

        // check if no email column is selected
        if( $('[name="slb_email_column"] option:checked', $import_form_2).val() == '' )
            is_valid = false;
